...
import type { Campaign, CampaignStatusData, LastRecipient, WorkerActivity } from "@/types/campaign"
...
function CampaignStatus({ campaign, statusData }: CampaignStatusProps) {
  ...
  const progressPercentage = Math.min(100, Math.max(0, statusData?.progreso?.porcentaje ?? 0))
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center p-3 border rounded-lg">
                ...
                <div className="text-xs md:text-sm text-muted-foreground">Pendientes</div>
              </div>
              <div className="text-center p-3 border rounded-lg">
                <div className="text-xl md:text-2xl font-bold text-green-600">{statusData.resumen?.enviado || 0}</div>
                <div className="text-xs md:text-sm text-muted-foreground">Enviados</div>
              </div>
              <div className="text-center p-3 border rounded-lg">
                <div className="text-xl md:text-2xl font-bold text-red-600">{statusData.resumen?.rebotados || 0}</div>
                <div className="text-xs md:text-sm text-muted-foreground">Rebotes</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-3 bg-purple-50 dark:bg-purple-900/30 rounded-lg">
                ...
                <div className="text-xs md:text-sm text-muted-foreground">Abiertos</div>
              </div>
              <div className="text-center p-3 bg-orange-50 dark:bg-orange-900/30 rounded-lg">
                <div className="text-xl md:text-2xl font-bold text-orange-600">
                  {statusData.metricas?.clic_unicos || 0}
                </div>
                <div className="text-xs md:text-sm text-muted-foreground">Clics</div>
              </div>
              <div className="text-center p-3 bg-gray-50 dark:bg-gray-900/30 rounded-lg">
                <div className="text-xl md:text-2xl font-bold text-gray-600">{statusData.resumen?.bajas || 0}</div>
                <div className="text-xs md:text-sm text-muted-foreground">Bajas</div>
              </div>
              <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
                <div className="text-xl md:text-2xl font-bold text-blue-600">
                  {statusData.metricas?.open_rate_uni_pct || 0}%
                </div>
                <div className="text-xs md:text-sm text-muted-foreground">Tasa Apertura</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-2 text-xs">
              ...
              <div className="text-center p-2 bg-green-100 dark:bg-green-900/30 rounded">
                <div className="font-bold">{statusData.resumen?.enviado || 0}</div>
              ...
              ultimos.map((evento) => (
                <div key={evento.id} className="flex items-center justify-between p-2 border rounded-lg">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">{fmtDateTime(evento.actualizado_at)}</div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Badge className={`text-xs ${getEstadoColor(evento.estado)}`}>{evento.estado}</Badge>
                    {evento.aperturas > 0 && (
...
function WorkerActivities({ actividades }: { actividades: WorkerActivity[] }) {
  const etapaColor = (etapa: WorkerActivity["etapa"], ok: boolean) => {
    if (!ok) return "text-red-600 bg-red-100 dark:text-red-400 dark:bg-red-900/30"
    switch (etapa) {
      case "siguiente":
        return "text-blue-600 bg-blue-100 dark:text-blue-400 dark:bg-blue-900/30"
      case "render":
        ...
        <ScrollArea className="h-64">
          <div className="space-y-2">
            {!actividades || actividades.length === 0 ? (
              <div className="text-center text-muted-foreground py-4">No hay actividades registradas aún</div>
            ) : (
              actividades
                .slice()
                .reverse()
                .map((a) => (
                  <div key={a.id} className="flex items-center justify-between p-2 border rounded-lg">
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge className={`text-xs ${etapaColor(a.etapa, a.ok)}`}>{a.etapa}</Badge>
                      {!a.ok && a.error && (
                        <Badge variant="outline" className="text-xs">
                          {a.error}
                        </Badge>
...
export function SendCampaign() {
  ...
  const rawActs: WorkerActivity[] =
    (statusData?.actividades as WorkerActivity[]) || (statusData?.llamadas as WorkerActivity[]) || []
  ...
  const actividadesEstables = rawActs.length > 0 ? rawActs : lastNonEmptyActsRef.current
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-xl md:text-2xl font-bold">Ver envío de campaña</h2>
            <p className="text-sm md:text-base text-muted-foreground">No hay envíos activos ahora mismo</p>
          </div>
          <Button onClick={loadActiveCampaign} variant="outline" className="self-start sm:self-auto bg-transparent">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-bold">Gestión de Campaña</h2>
      ...
      {statusData?.campana?.proximo_envio_at && activeCampaign.estado === "en_ejecucion" && (
        <Alert>
          ...
          <AlertDescription>
            Próximo envío programado para: {fmtDateTime(statusData.campana.proximo_envio_at)}
                ...
                <CardTitle className="flex items-center">
                  <Heart
                    className={`w-5 h-5 mr-2 ${
                      statusData.worker.alive ? "text-green-500 animate-pulse" : "text-red-500"
                    }`}
                  />
                  Estado del Worker
                </CardTitle>
